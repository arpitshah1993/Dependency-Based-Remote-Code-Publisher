...
namespace CodeAnalysis
{
	...
	{
	private:
		NoSqlDb<DepElement> nameSpaceTable;
		NoSqlDb<TypeElement> typeTable;
		static ServerRepo* instance;
	public:
		ServerRepo()
		{
			instance = this;
		...
		static ServerRepo* getInstance()
		{
			//instance = new ServerRepo;
			return instance;
		}
		NoSqlDb<TypeElement>& getTT() { return typeTable; }
		NoSqlDb<DepElement>& getNameSpaceTable() { return nameSpaceTable; }
	};

	// ServerRepo* ServerRepo::instance=nullptr;

	///////////////////////////////////////////////////////////////////
	// Repository instance is used to share resources
	// among all actions.
	// using NamespaceTable = vector<std::string, vector<std::string>>;
	...
	private:
		...
		NoSqlDb<DepElement> depAnal;
		NoSqlDb<DepElement>& nameSpaceTable;
	public:

		Repository(Scanner::Toker* pToker) : ast(stack),
			typeTable(ServerRepo::getInstance()->getTT()),
			nameSpaceTable(ServerRepo::getInstance()->getNameSpaceTable())
		{
			p_Toker = pToker;
		...
		NoSqlDb<TypeElement>& getTT() { return typeTable; }
		NoSqlDb<DepElement>& getdepAnal() { return depAnal; }
		NoSqlDb<DepElement>& getNameSpaceTable() { return nameSpaceTable; }
	...
	public:
		...
		void doAction(const Scanner::ITokCollection* pTc) override
		{
			...
			if (nameIndex > 1 && (*pTc)[nameIndex - 1] == "::") {
				...
				//adding reverse h->cpp dependency
				NoSqlDb<TypeElement>& typeAnalRef_(CodeAnalysis::Repository::getInstance()->getTT());
				TypeElement elem("zaxscdvf", pFunctNode->path_+"|"+ pClassNode->path_, "class");
				typeAnalRef_.saveOrUpdate(className, elem);
				return;
			}
