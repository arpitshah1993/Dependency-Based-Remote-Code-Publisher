...
Public Interface:
=================
XToker t;                        // create tokenizer instance
...
- devenv ArpitShahHW1.sln /debug rebuild
...
namespace XmlProcessing
{
	...
	public:
		...
		XToker(const std::string& src = "", bool isFile = true);
		~XToker();
	...
	private:
		...
		XToker(const XToker &tkr);
		XToker& operator=(const XToker&);
	};

	inline void XToker::returnComments(bool doReturn)
	{
		doReturnComments = doReturn;
	}

	inline bool XToker::isFileEnd() { return (nextChar == -1); }

	inline int& XToker::lines() { return numLines; }

	inline int XToker::braceLevel() { return braceCount; }
}
#endif
