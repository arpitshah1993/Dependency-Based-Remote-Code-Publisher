...
#include"../TypeAnalyzer/TypeAnal.h"
#include"../DependencyAnalyzer/DepAnaly.h"
...
{
	std::ostringstream out;
	out << "\n     Path: \"" << FileSystem::Path::getFullFileSpec(argv[3]) << "\"\n     Args: ";
...
bool CodeAnalysisExecutive::ProcessCommandLine(int argc, char* argv[])
{
	...
	try {
		path_ = FileSystem::Path::getFullFileSpec(argv[3]);
...
//----< ANalysing file to publish >----------------------------------------------------
void CodeAnalysisExecutive::AnalyseFileToPublish(CodeAnalysisExecutive& exectemp, bool showProc, NoSqlDb<DepElement>& depAnalRef_, string& file, Publisher& p,string& publishPath)
{
	vector<int> start, end;
	exectemp.processSourceCodeAnalysing(file, showProc);
	ASTNode* pNode = exectemp.pRepo_->getInstance()->AST().root();
	...
	while (stl.size() != 0)
	{
		...
		{
			stl.push(pChild);
			if (pChild->type_ == "lambda" || pChild->type_ == "function" || pChild->type_ == "class" || pChild->type_ == "interface" || pChild->type_ == "struct")
			{
				start.push_back((int)pChild->startLineCount_);
				end.push_back((int)pChild->endLineCount_);
			}
		}
	}
	p.publishHtml(file, start, end, depAnalRef_, publishPath);
	std::cout << "\n     Done!";
}

//----< Processing publishing the code files >----------------------------------------------------
void CodeAnalysisExecutive::procesPublishing(string file, string publishPath)
{
	std::cout << "\n  Publishing Process Started\n";
	int i = 0;
	NoSqlDb<DepElement>& depAnalRef_(CodeAnalysis::Repository::getInstance()->getdepAnal());
	std::cout << "\n  Reading Predefined html helper text files\n";
	Publisher p;
	std::cout << "\n  Started code files publishing.\n";
	string dependentFileName;
	/*for (auto file : cppHeaderFiles())
	{
		std::cout << std::endl << "  " << ++i;
		CodeAnalysisExecutive exectemp;
		exectemp.pRepo_->language() = Language::Cpp;
		AnalyseFileToPublish(exectemp, showProc, depAnalRef_, file, p);
		dependentFileName = FileSystem::Path::getName(file, true);
		out << "<a href=\"" << dependentFileName + ".htm\">" << dependentFileName << "</a><br>";
	}
	...
	{*/
		CodeAnalysisExecutive exectemp;
		exectemp.pRepo_->language() = Language::CSharp;
		AnalyseFileToPublish(exectemp, true, depAnalRef_, file, p, publishPath);
	std::cout << std::endl;
...
//----< Processs source code and do depenedcy analysis >--------------
void CodeAnalysisExecutive::initProcessSourceCodeDepAnal(bool showProc)
{
	std::function<std::string(std::string, size_t)> trunc = [](std::string in, size_t count)
	{
		return in.substr(0, count);
	...
	DepAnal da;
	//NoSqlDb<DepElement>& depAnalRef_(Repository::getInstance()->getdepAnal());
...
//----< Processs source code and do depenedcy analysis >--------------
void CodeAnalysisExecutive::processSourceCodeDepAnal(string file)
{
	std::function<std::string(std::string, size_t)> trunc = [](std::string in, size_t count)
	{
		return in.substr(0, count);
...
//----< Processs source code in part>--------------
void CodeAnalysisExecutive::processSourceCodeAnalysing(string file, bool showProc)
{
	if (showProc)
		showActivity(file);
...
//----< Processs source code for all file>--------------
void CodeAnalysisExecutive::initprocessSourceCode(bool showProc, unordered_map<string, bool>& fileState)
{
	for (auto file : cppHeaderFiles())
	{
		...
		pRepo_->language() = Language::Cpp;
		processSourceCodeAnalysing(file, showProc);
	}
	for (auto file : cppImplemFiles())
	{
		std::cout << "\n  processing file: " << file;
...
//----< Processs source code for particular file>--------------
void CodeAnalysisExecutive::processSourceCode(string file)
{
	//for (auto file : cppHeaderFiles())
	//{
	...
	{
		pRepo_->language() = Language::CSharp;
		processSourceCodeAnalysing(file, true);
	}
	else 
	{
		pRepo_->language() = Language::Cpp;
...
//----< Initialis code analysis >-------------------------------
void CodeAnalysisExecutive::codeAnalysisInitialisation(int argc, char* argv[],unordered_map<string, bool>& fileState)
{
	...
	getSourceFiles();
	initprocessSourceCode(true, fileState);
	complexityAnalysis();
	dispatchOptionalDisplays();
	flushLogger();
	displayMetricSummary(50, 10);
...
//----< Initialis code analysis >-------------------------------
void CodeAnalysisExecutive::codeAnalysisInitialisation(string file)
{
	//std::cout << "\n  Code Analysis Started";
	//bool succeeded = ProcessCommandLine(argc, argv);
	//if (!succeeded)
...
//----< Initialis type analysis >-------------------------------
void CodeAnalysis::CodeAnalysisExecutive::typeAnalysisProcess()
{
...
int main(int argc, char* argv[])
{
	...
	{
		argvb.push_back(argv[i]);
	}
	try {
		exec.processSourceCodeDepAnal(true);
		int ipo;
		std::cin >> ipo;
